      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-5 p-6 md:flex-row md:items-end md:justify-between md:p-8">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-xs font-extrabold text-violet-700">
                <Sparkles className="h-3.5 w-3.5" />
                MODO DE PRUEBA · IA SIMULADA
              </div>
              <h1 className="text-3xl font-black tracking-tight text-slate-950 md:text-4xl">Reuniones</h1>
            ...
            <button
              type="button"
              disabled
              className="inline-flex h-11 cursor-not-allowed items-center justify-center rounded-xl bg-slate-200 px-4 text-sm font-bold text-slate-500"
          ...
          <div className="grid gap-3 border-t border-slate-200 bg-slate-50/70 p-4 md:grid-cols-[1fr_220px] md:p-5">
            <div className="relative">
              ...
              <input
                ...
                className="h-11 w-full rounded-xl border border-slate-300 bg-white pl-10 pr-3 text-sm outline-none transition focus:border-red-500 focus:ring-4 focus:ring-red-100"
              />
            </div>
            <select
              value={branch}
              onChange={(event) => setBranch(event.target.value)}
              className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-800 outline-none focus:border-red-500"
            >
          ...
          {filtered.map((meeting) => (
            <article key={meeting.id} className="group flex min-h-[290px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md">
              <div className="flex items-start justify-between gap-3">
                <div className="flex flex-wrap gap-2">
                  {meeting.branches.map((item) => (
                    <span key={item} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-extrabold text-slate-700">
                      <Building2 className="h-3 w-3" /> {item}
                    </span>
                  ))}
                </div>
                <span className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-emerald-700">Procesada</span>
              </div>

              <h2 className="mt-5 text-xl font-black leading-tight text-slate-950">{meeting.title}</h2>
              <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600">{meeting.summary}</p>
              ...
              <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-4">
                <div className="text-xs font-semibold text-slate-500">{meeting.keyPoints.length} puntos · {meeting.actions.length} acciones</div>
                <Link href={`/reuniones/${meeting.id}`} className="inline-flex items-center gap-1.5 text-sm font-extrabold text-slate-900 transition group-hover:text-red-600">
