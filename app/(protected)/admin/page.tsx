...
setRows((data ?? []) as Row[]);
  ...
  const saveRow = async (u: Row) => {
    ...
    const { error: e1 } = await supabase.rpc('admin_upsert_profile', {
      p_user_id: u.id, p_full_name: u.full_name, p_role: u.role, p_is_active: u.is_active,
    });
    if (e1) { setSaving(null); showToast('error', e1.message); return; }
    const { error: e2 } = await supabase.rpc('admin_set_user_branches', {
      p_user_id: u.id, p_branches: u.branches,
    });
    if (e2) { setSaving(null); showToast('error', e2.message); return; }
    setSaving(null);
    await load();
  ...
  const confirmToggleActive = async () => {
    ...
    const { error: e1 } = await supabase.rpc('admin_upsert_profile', {
      ...
      p_full_name: targetUser.full_name,
    ...
    setRows(prev => prev.map(r => r.id === targetUser.id ? { ...r, is_active: desired } : r));
    await load();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      {/* Toast */}
      {toast && (
        <div className={`fixed right-4 top-4 z-[60] rounded-lg px-4 py-3 shadow-lg border ${
          toast.type === 'success'
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <select
                        ...
                        value={u.role}
                        onChange={(e)=>!disabled && setRows(prev=>prev.map(r=>r.id===u.id?{...u, role: e.target.value as any}:r))}
                        disabled={disabled}
                      >
