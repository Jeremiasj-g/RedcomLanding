...
export default function AdminSolicitudesPage() {
    ...
    const show = (t: 'ok' | 'err', msg: string) => { setToast({ type: t, msg }); setTimeout(() => setToast(null), 3000); };
    ...
    const load = async () => {
        ...
        const { data, error } = await supabase
            .from('signup_requests')
            .select('id,email,status,created_at,processed_at,processed_by,requested_branches,comment')
        ...
        if (ids.length) {
            ...
            (profs ?? []).forEach((p: any) => { map[p.id] = { id: p.id, full_name: p.full_name, email: p.email }; });
            setApprovers(map);
        } else {
            setApprovers({});
        }
    ...
    const openApprove = (r: Req) => {
        ...
        setRole('user');
        setIsActive(true);
        setBranches(r.requested_branches ?? []);   // <<<<<< pre-selección
        setOpen(true);
    };

    const approve = async () => {
        if (!target) return;
        if (password.length < 8) { show('err', 'La contraseña debe tener al menos 8 caracteres'); return; }
        if (branches.length === 0) { show('err', 'Seleccioná al menos una sucursal'); return; }
        ...
        if (!res.ok) {
            ...
            setSaving(false);
            show('err', j.error ?? 'No se pudo crear el usuario');
            return;
        }
    ...
    const badge = (status: Req['status']) => {
        ...
        return <span className={`px-2.5 py-1 rounded-lg text-xs font-medium ${m[status]}`}>{label[status]}</span>;
    };
    ...
    return (
        <div className="max-w-7xl mx-auto py-6">
            {toast && (
                <div className={`fixed right-4 top-4 z-50 rounded-lg px-4 py-2 shadow ${toast.type === 'ok' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
                    {toast.msg}
                </div>
            )}
            ...
            <div className="mb-6 flex items-end justify-between gap-3">
                <div>
                    ...
                    <p className="text-slate-600">Aprobá o rechazá solicitudes y revisá el historial.</p>
                </div>
                <div className="flex items-center gap-2">
                    {(['pending', 'approved', 'rejected', 'all'] as const).map(s => (
                        <button
                            ...
                            className={`rounded-lg px-3 py-2 text-sm border ${statusFilter === s ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-700 hover:bg-slate-50'}`}
                        >
            ...
            <div className="space-y-3">
                {filtered.map(r => {
                    ...
                    return (
                        <div key={r.id} className="rounded-xl border p-4 bg-white flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                            <div className="min-w-0">
                                ...
                                <div className="mt-1 text-xs text-slate-500">
                                    ...
                                    {r.processed_at && (
                                        <>
                                            ...
                                            {approver && <> {' • '}por {approver.full_name ? `${approver.full_name} (${approver.email})` : approver.email}</>}
                                        </>
                                    )}
                                </div>
                                ...
                                {r.requested_branches?.length > 0 && (
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        {r.requested_branches.map(b => (
                                            <span key={b} className="rounded-full border px-2.5 py-0.5 text-xs bg-slate-50 text-slate-700">
                                                {b}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                {r.comment && (
                                    <p className="mt-2 text-sm text-slate-700">
                                        <span className="text-slate-500 text-xs uppercase tracking-wide">Comentario: </span>
                                        {r.comment}
                                    </p>
                                )}
                            </div>
                            ...
                            <div className="flex gap-2 shrink-0">
                                ...
                                {/* Si también querés permitir reabrir aprobadas, descomentá:
                                    {r.status === 'approved' && (
                                        <button
                                        className="rounded-lg border px-3 py-2 hover:bg-slate-50"
                                        onClick={() => reopen(r)}
                ...
                {filtered.length === 0 && (
                    <div className="rounded-xl border p-10 text-center text-slate-500 bg-white">No hay solicitudes.</div>
                )}
            </div>
            ...
            {open && target && (
                <div className="fixed inset-0 z-50 grid place-items-center">
                    ...
                    <div className="relative z-10 w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
                        ...
                        <p className="text-sm text-slate-600">Se creará el usuario para <b>{target.email}</b>.</p>

                        <div className="mt-4 space-y-3">
                            <div>
                                <label className="text-xs text-slate-600">Nombre completo</label>
                                <input className="w-full rounded-xl border px-3 py-2"
                                    value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Ej: Juan Pérez" />
                            </div>
                            <div>
                                <label className="text-xs text-slate-600">Contraseña (min 8)</label>
                            ...
                            <div className="flex items-center gap-3">
                                <div>
                                    ...
                                    <select className="ml-2 rounded-lg border px-2 py-1"
                            ...
                            <div>
                                <div className="text-xs text-slate-600 mb-2">Sucursales</div>
                                <div className="flex flex-wrap gap-2">
                                    {ALL_BRANCHES.map(b => (
                                        <button key={b} type="button"
                                            onClick={() => toggleBranch(b)}
                                            className={`rounded-full border px-3 py-1 text-sm ${branches.includes(b) ? 'bg-emerald-100 border-emerald-300' : 'hover:bg-slate-50'}`}>
                                            {b}
                                        </button>
                                    ))}
                                </div>
                        ...
                        <div className="mt-5 flex justify-end gap-2">
                            ...
                            <button className={`rounded-lg px-4 py-2 text-white ${saving ? 'opacity-60' : 'bg-slate-900 hover:bg-slate-800'}`}
                ...
                <div className="fixed inset-0 z-50 grid place-items-center">
                    ...
                    <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border">
                        <div className="flex items-start gap-3">
                            ...
                            <div className="min-w-0">
                                <h3 className="text-lg font-semibold text-slate-900">Rechazar solicitud</h3>
                                <p className="mt-1 text-sm text-slate-600">
                                    ¿Estás seguro de rechazar la solicitud de <b>{rejectTarget.email}</b>?<br />
