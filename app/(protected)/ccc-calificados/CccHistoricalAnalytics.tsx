...
type Summary = {
  ...
  closedAt: string;
  generatedAt: string;
  totals: { clients: number; sellers: number; supervisors: number };
  brands: Metric[];
  supervisors: GroupMetric[];
  vendors: GroupMetric[];
};

type PeriodResult = { id: number; summary: Summary | null; error?: string };

function currentBranch() {
  if (typeof window === "undefined") return "";
  const params = new URLSearchParams(window.location.search);
  return String(
    params.get("ccc_branch") || window.localStorage.getItem(CCC_LAST_BRANCH_KEY) || "",
  )
    .trim()
    .toLowerCase();
}

function periodLabel(summary: Summary) {
  return `${MONTHS[Math.max(0, summary.periodMonth - 1)]} ${summary.periodYear}`;
}
...
function findBrand(summary: Summary | undefined, code: string) {
...
function EmptyAnalytics({ branchLabel }: { branchLabel: string }) {
  return (
...
function AnalyticsDashboard({ branch, periods, refresh, loading }: { branch: string; periods: PeriodResult[]; refresh: () => void; loading: boolean }) {
  const valid = useMemo(
    () => periods.filter((period): period is { id: number; summary: Summary } => Boolean(period.summary)).sort((a, b) => {
      const x = a.summary.periodYear * 100 + a.summary.periodMonth;
      const y = b.summary.periodYear * 100 + b.summary.periodMonth;
      return x - y;
    }),
    [periods],
  );
  ...
  const [ranking, setRanking] = useState<"supervisor" | "vendor">("supervisor");

  const brandOptions = useMemo(() => {
    const map = new Map<string, string>();
    valid.forEach(({ summary }) => summary.brands.forEach((brand) => map.set(brand.code, brand.label)));
  ...
  const trend = useMemo(
    () => visible.map(({ id, summary }) => {
      ...
      return {
        ...
        coverage: brand?.coveragePct ?? 0,
        purchase: brand?.purchasePct ?? 0,
        qualified: brand?.qualified ?? 0,
        buyers: brand?.buyers ?? 0,
        clients: brand?.clients ?? summary.totals.clients,
        units: brand?.units ?? 0,
        mix: brand?.avgMixBuyer ?? 0,
        dropsize: brand?.dropsize ?? null,
      };
    }),
    [brandCode, visible],
  );
    ...
    return source
      .map((item) => {
        ...
        return {
          ...
          subtitle: ranking === "vendor" ? String(item.supervisor || "") : `${item.sellers || 0} vendedores`,
          coverage: brand?.coveragePct ?? 0,
          qualified: brand?.qualified ?? 0,
          units: brand?.units ?? 0,
        };
  ...
  const brandComparison = useMemo(
    () => (focus?.summary.brands || []).map((brand) => ({
      brand: brand.label,
      coverage: brand.coveragePct,
      purchase: brand.purchasePct,
      units: brand.units,
    ...
    <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 max-w-2xl text-xs leading-5 text-slate-500">
                Compara los períodos congelados del CCC. Los resúmenes quedan cacheados para no volver a descargar los snapshots completos.
              </p>
            </div>
          </div>
          <button type="button" onClick={refresh} disabled={loading} className="inline-flex h-9 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-2"><DeltaBadge value={delta(focusBrand?.coveragePct, previousBrand?.coveragePct)} suffix=" pp" /></div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-2"><DeltaBadge value={delta(focusBrand?.qualified, previousBrand?.qualified)} /></div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-2"><DeltaBadge value={delta(focusBrand?.units, previousBrand?.units)} /></div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-2"><DeltaBadge value={delta(focusBrand?.dropsize, previousBrand?.dropsize)} /></div>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
          ...
          <div className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend} margin={{ top: 8, right: 16, bottom: 4, left: 0 }}>
                ...
                <Tooltip formatter={(value: any, name: any) => [`${formatNumber(Number(value), 1)}%`, name === "coverage" ? "Cumplimiento" : "Con compra"]} />
          ...
          <div className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trend} margin={{ top: 8, right: 16, bottom: 4, left: 4 }}>
                <defs><linearGradient id="cccUnitsArea" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563eb" stopOpacity={0.32}/><stop offset="95%" stopColor="#2563eb" stopOpacity={0.03}/></linearGradient></defs>
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="period" tick={{ fontSize: 11 }} />
                <YAxis tick={{ fontSize: 11 }} tickFormatter={(value) => formatCompact(Number(value))} />
                <Tooltip formatter={(value: any) => [formatNumber(Number(value)), "Unidades"]} />
        ...
        <ChartCard title="Cartera, compradores y calificados" subtitle="Muestra cómo se transforma la cartera total en compra y luego en cumplimiento.">
          <div className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={trend} margin={{ top: 8, right: 12, bottom: 4, left: 0 }}>
                ...
                <Tooltip formatter={(value: any, name: any) => [formatNumber(Number(value)), name === "clients" ? "Cartera" : name === "buyers" ? "Compradores" : "Cumplieron"]} />
        ...
        <ChartCard title="Dropsize histórico" subtitle="Unidades por comprobante para la marca seleccionada, cuando el cierre contiene DROPSIZE.">
          <div className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend} margin={{ top: 8, right: 16, bottom: 4, left: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} />
                ...
                <Tooltip formatter={(value: any) => [value == null ? "—" : formatNumber(Number(value), 2), "Dropsize"]} />
          ...
          <div className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend} margin={{ top: 8, right: 16, bottom: 4, left: 0 }}>
                ...
                <Tooltip formatter={(value: any) => [formatNumber(Number(value), 2), "MIX promedio"]} />
        ...
        <ChartCard title={`Comparativa de marcas · ${focus ? periodLabel(focus.summary) : "—"}`} subtitle="Cumplimiento y penetración de compra de todas las marcas configuradas en ese cierre.">
          <div className="h-[310px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <ChartCard title={`Ranking comercial · ${focus ? periodLabel(focus.summary) : "—"}`} subtitle="Ordena de mayor a menor según cumplimiento CCC para la marca seleccionada.">
        ...
        <div style={{ height: Math.max(320, rankingRows.length * 42) }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rankingRows} layout="vertical" margin={{ top: 4, right: 24, bottom: 4, left: 18 }}>
              ...
              <Tooltip formatter={(value: any) => [`${formatNumber(Number(value), 1)}%`, "Cumplimiento CCC"]} />
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 px-5 py-4">
          ...
          <p className="mt-1 text-xs text-slate-500">Una lectura rápida de la evolución de la marca seleccionada.</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] text-left text-xs">
            <thead className="bg-slate-50 text-[10px] font-semibold uppercase tracking-[0.06em] text-slate-400">
              <tr><th className="px-4 py-3">Período</th><th className="px-4 py-3 text-right">Cartera</th><th className="px-4 py-3 text-right">Compradores</th><th className="px-4 py-3 text-right">Cumplieron</th><th className="px-4 py-3 text-right">CCC %</th><th className="px-4 py-3 text-right">Unidades</th><th className="px-4 py-3 text-right">MIX</th><th className="px-4 py-3 text-right">Dropsize</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {[...visible].reverse().map(({ id, summary }) => {
                const brand = findBrand(summary, brandCode);
                return <tr key={id} className="hover:bg-slate-50/70"><td className="px-4 py-3 font-semibold text-slate-800">{periodLabel(summary)}</td><td className="px-4 py-3 text-right text-slate-600">{formatNumber(brand?.clients || 0)}</td><td className="px-4 py-3 text-right text-slate-600">{formatNumber(brand?.buyers || 0)}</td><td className="px-4 py-3 text-right text-slate-600">{formatNumber(brand?.qualified || 0)}</td><td className="px-4 py-3 text-right font-semibold text-slate-900">{formatNumber(brand?.coveragePct || 0, 1)}%</td><td className="px-4 py-3 text-right text-slate-600">{formatNumber(brand?.units || 0)}</td><td className="px-4 py-3 text-right text-slate-600">{formatNumber(brand?.avgMixBuyer || 0, 2)}</td><td className="px-4 py-3 text-right text-slate-600">{brand?.dropsize == null ? "—" : formatNumber(brand.dropsize, 2)}</td></tr>;
              })}
            </tbody>
          </table>
        </div>
...
export default function CccHistoricalAnalytics() {
  ...
  const load = useCallback(async (targetBranch?: string) => {
  ...
  useEffect(() => {
    const install = () => {
      const nav = document.querySelector<HTMLElement>("nav.ccc-tabs");
  ...
  useEffect(() => {
    const nav = document.querySelector<HTMLElement>("nav.ccc-tabs");
    const panels = Array.from(document.querySelectorAll<HTMLElement>(".ccc-tab-panel"));
    if (!nav) return;
  ...
  const tab = tabHost ? createPortal(
    <button id="ccc-tab-analytics" type="button" role="tab" aria-selected={active} aria-controls="ccc-panel-analytics" className={`ccc-analytics-tab ${active ? "is-active" : ""}`} onClick={() => setActive(true)}>
    ...
    <section id="ccc-panel-analytics" role="tabpanel" aria-labelledby="ccc-tab-analytics" className={active ? "block" : "hidden"}>
      ...
      {loading && !periods.length ? (
        <div className="ccc-historical-loading grid place-items-center rounded-2xl border border-slate-200 bg-white">
          <div className="text-center"><RefreshCw className="mx-auto h-6 w-6 animate-spin text-slate-500" /><p className="mt-3 text-sm font-semibold text-slate-800">Preparando analítica histórica…</p><p className="mt-1 text-xs text-slate-500">Los cierres antiguos se resumen una sola vez y luego quedan cacheados.</p></div>
        </div>
      ) : error ? (
