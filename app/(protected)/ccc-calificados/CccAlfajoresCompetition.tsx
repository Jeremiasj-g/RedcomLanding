...
import {
  downloadMixAlfajoresFileByMeta,
  listMixAlfajoresFiles,
...
type CompetitionResult = {
  ...
  reports: CccMixAlfajoresFileMeta[];
...
async function buildCompetitionResult(branch: string): Promise<CompetitionResult> {
  ...
  const register = (
    map: Map<string, typeof sellerList>,
    key: string,
    seller: (typeof sellerList)[number],
  ) => {
    if (!key) return;
  ...
  const downloadedReports = await Promise.all(
    reports.map((meta) => downloadMixAlfajoresFileByMeta(meta)),
  );

  for (const report of downloadedReports) {
    const workbook = XLSX.read(await report.file.arrayBuffer(), {
      type: "array",
      cellDates: true,
    });
    ...
    for (let rowIndex = headerRow + 1; rowIndex < rows.length; rowIndex += 1) {
      ...
      const seller = resolveSeller(vendorName, row[vendorCodeIndex]);
      ...
      const vendorKey = `${seller.codigo || row[vendorCodeIndex] || ""}|${normalizeName(seller.nombre || vendorName)}`;
      ...
      vendor.clients.add(clientCode || normalize(clientName));
    }
  }
  ...
  const totalClients = new Set(
    allVendorEntries.flatMap((vendor) => Array.from(vendor.clients)),
  ).size;
  ...
  return {
    ...
    reports,
    latestUpdate: reports[0]?.updated_at || reports[0]?.uploaded_at || null,
  };
}
...
export default function CccAlfajoresCompetition() {
  ...
  useEffect(() => {
    const syncBranch = () => setBranch(currentBranch());
    syncBranch();
    window.addEventListener("ccc:branch-changed", syncBranch);
    return () => window.removeEventListener("ccc:branch-changed", syncBranch);
  }, []);
  ...
  useEffect(() => {
    const install = () => {
      ...
      if (!host) {
        ...
        if (snapshotHost?.parentElement === navActions) {
          snapshotHost.insertAdjacentElement("afterend", host);
        } else {
  ...
  const refresh = useCallback(async () => {
    ...
    setError("");
    try {
      const next = await buildCompetitionResult(branch);
      setResult(next);
    } catch (cause: any) {
      console.error(cause);
      setResult(null);
        ...
        <div className="space-y-4 pb-6">
            ...
            <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <h2 className="mt-1 text-xl font-semibold">Resultados · {branchLabel}</h2>
                  <p className="mt-1 text-xs leading-5 text-slate-400">
                    Consolidado de los reportes MIX Alfajores cargados por el equipo. Cierre: 30 de septiembre de 2026.
                  </p>
                </div>
              </div>
          ...
          {loading ? (
            <div className="grid min-h-[320px] place-items-center rounded-xl border border-slate-200 bg-white">
              <div className="flex flex-col items-center gap-3 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">Leyendo los reportes cargados por la sucursal.</p>
                </div>
              </div>
            </div>
          ) : error ? (
            ...
            <div className="rounded-xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center">
              ...
              <p className="mt-1 text-xs text-slate-500">Los resultados aparecerán cuando los supervisores carguen su reporte desde MIX Alfajores.</p>
            </div>
          ) : (
            <>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                <div className="kpi-card" style={{ ["--kc" as any]: "#C8102E" }}>
                  ...
                  <div className="k-sub">cantidad consolidada de la sucursal</div>
                </div>
              ...
              <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div className="flex flex-col gap-2 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">Supervisores ordenados por MIX vendido, con todos sus vendedores visibles.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => void refresh()}
                    className="inline-flex h-9 items-center gap-2 self-start rounded-lg border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50"
                  >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[760px] border-collapse text-xs">
                    <thead className="bg-slate-900 text-white">
                      <tr>
                        <th className="w-24 px-4 py-3 text-center font-semibold">Pos.</th>
                        <th className="px-4 py-3 text-left font-semibold">Supervisor / Vendedor</th>
                        <th className="w-28 px-4 py-3 text-right font-semibold">Clientes</th>
                        <th className="w-36 px-4 py-3 text-right font-semibold">MIX vendido</th>
                        <th className="w-32 px-4 py-3 text-right font-semibold">Participación</th>
                      </tr>
                    </thead>
                    <tbody>
                      {result.supervisors.map((supervisor, index) => {
                        const rank = index + 1;
                        const share = result.totalMix ? (supervisor.mix / result.totalMix) * 100 : 0;
                        return [
                          <tr key={`supervisor:${supervisor.name}`} className="border-t border-slate-200 bg-slate-50">
                            <td className="px-4 py-3 text-center">
                              <span className={`inline-flex min-w-9 items-center justify-center rounded-full border px-2 py-1 text-[11px] font-bold ${rankBadge(rank)}`}>
                                #{rank}
                              </span>
                            </td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  ...
                                  <div className="font-bold text-slate-950">{supervisor.name}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-right font-semibold text-slate-700">{formatNumber(supervisor.clients)}</td>
                            <td className="px-4 py-3 text-right text-sm font-bold text-slate-950">{formatNumber(supervisor.mix, 2)}</td>
                            <td className="px-4 py-3 text-right font-semibold text-slate-600">{formatNumber(share, 1)}%</td>
                          </tr>,
                          ...supervisor.vendors.map((vendor) => (
                            <tr key={`vendor:${supervisor.name}:${vendor.key}`} className="border-t border-slate-100 hover:bg-slate-50/70">
                              <td className="px-4 py-2.5 text-center text-slate-300">—</td>
                              <td className="px-4 py-2.5 pl-12">
                                <div className="text-[9px] font-semibold uppercase tracking-wide text-slate-400">Vendedor</div>
                                <div className="font-semibold text-slate-700">{vendor.name}</div>
                              </td>
                              <td className="px-4 py-2.5 text-right font-medium text-slate-600">{formatNumber(vendor.clients)}</td>
                              <td className="px-4 py-2.5 text-right font-semibold text-slate-800">{formatNumber(vendor.mix, 2)}</td>
                              <td className="px-4 py-2.5 text-right text-slate-500">
                                {formatNumber(supervisor.mix ? (vendor.mix / supervisor.mix) * 100 : 0, 1)}%
                              </td>
                            </tr>
                          )),
                        ];
