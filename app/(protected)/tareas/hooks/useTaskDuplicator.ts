...
export function useTaskDuplicator() {
  ...
  async function duplicate(task: Task, dateYMD: string, timeHHmm: string) {
    ...
    const created = await createTask({ title: task.title, description: task.description ?? undefined, scheduled_at });
    setTasks((prev) => [...prev, created]);
    return created;
  }

  async function duplicateRange(task: Task, fromYMD: string, toYMD: string, timeHHmm: string) {
